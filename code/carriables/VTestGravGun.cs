...
{
	[Library( "gravgun" )]
	public partial class VTestGravGun : VTestCarriable
	{
		...
		private static bool IsBodyGrabbed( PhysicsBody body )
		{
			...
			if ( All.OfType<VTestPhysGun>().Any( x => x?.HeldBody?.PhysicsGroup == body?.PhysicsGroup ) ) return true;
			if ( All.OfType<VTestGravGun>().Any( x => x?.HeldBody?.PhysicsGroup == body?.PhysicsGroup ) ) return true;
