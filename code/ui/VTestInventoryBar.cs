...
{
	public class VTestInventoryBar : Panel
	{
		readonly List<VTestInventoryIcon> slots = new();

		public VTestInventoryBar()
		{
			for ( int i = 0; i < 9; i++ )
			{
				var icon = new VTestInventoryIcon( i + 1, this );
				slots.Add( icon );
			}
		}
		...
		private static void UpdateIcon( Entity ent, VTestInventoryIcon inventoryIcon, int i )
		{
			if ( ent == null )
			{
				inventoryIcon.Clear();
